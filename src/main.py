...
def main ():
    ...
    for symbol in Commodity_symbols:
        ...
        fd = fetch_Data()
        df = fd.getData(stock_symbol=symbol, start_date="2024-03-01", end_date="2025-01-15")
        ...
        axes[2].plot(visible[visible["signal"] == 1].index,visible["movingAverage_20"][visible["signal"] == 1], '^', markersize=9, color='green', label="20MA>50MA - buy signal")

        axes[2].plot(visible[visible["signal"] == -1].index, visible["movingAverage_50"][visible["signal"] == -1], 'v', markersize=9, color='red', label="20MA<50MA - sell signal")
        ...
        axes[2].plot(visible.index[regime_start], visible["close"][regime_start] * 0.995,'^', markersize=9,color='darkblue',linestyle='none',label="Let profits run (ATR low)",)


        axes[2].plot(visible.index[regime_end],visible["close"][regime_end] * 0.995,'v',markersize=9, color='darkred',linestyle='none',label="End of low volatility uptrend (ATR high)",)
